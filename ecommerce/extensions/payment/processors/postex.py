...
class PostExCOD(BasePaymentProcessor):
    """PostEx API implementation."""
    ...
    def __init__(self, site):
        """
        Constructs a new instance of the PostEx COD payment processor.
    ...
    def handle_processor_response(self, response, order_id=None, basket=None):
        """
        Record a successful PostEx COD payment.

        Arguments:
            response (dict): Dictionary of parameters returned by PostEx in COD payment request.
    ...
    def get_cities_list(self):
        ...
        try:
            cities = requests.get(self.configuration['get_cities_url'], headers={'token': self.configuration['key']})

            cities_list = []
            for city in cities.json().get('dist'):
                name = city.get('operationalCityName')
                cities_list.append(' '.join(name.capitalize() if not re.match(r'^\(.*[\)\*]*$', name) else name.upper() for name in name.split()))

            return sorted(cities_list)
        except Exception:  # pylint: disable=broad-except
            logger.exception(u"Unexpected error during cities list retrieval from PostEx COP API.")
            return "''"
